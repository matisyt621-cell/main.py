...
def draw_text_on_canvas(text, config, res=(1080, 1920), is_preview=False):
    ...
    combined.paste(shd_layer, (0,0), shd_layer); combined.paste(txt_layer, (0,0), txt_layer)
    if is_preview:
...
# --- 4. INTERFEJS ---
st.set_page_config(page_title="OMEGA UNIQUE", layout="wide")
st.title("Ω OMEGA V12.48 - UNIQUE COVERS")
...
with st.sidebar:
    ...
    config_dict = {'font_path': get_font_path(f_font), 'f_size': f_size, 't_color': t_color, 's_width': s_width, 's_color': s_color, 'shd_x': shd_x, 'shd_y': shd_y, 'shd_blur': shd_blur, 'shd_alpha': shd_alpha, 'shd_color': shd_color}
    if texts_list: st.image(draw_text_on_canvas(texts_list[0], config_dict, is_preview=True), use_container_width=True)

# --- 5. RENDERER Z LOGIKĄ UNIKALNYCH OKŁADEK ---
...
if st.button("🚀 GENERUJ UNIKALNE FILMY"):
    if u_cov and u_pho and texts_list:
        with st.status("🎬 Renderowanie...") as status:
            sid = int(time.time())
            c_p = []
            for i, f in enumerate(u_cov):
                p = f"c_{sid}_{i}.jpg"; with open(p, "wb") as b: b.write(f.getbuffer())
                c_p.append(p)
            p_p = []
            for i, f in enumerate(u_pho):
                p = f"p_{sid}_{i}.jpg"; with open(p, "wb") as b: b.write(f.getbuffer())
                p_p.append(p)
            m_p = []
            for i, f in enumerate(u_mus):
                p = f"m_{sid}_{i}.mp3"; with open(p, "wb") as b: b.write(f.getbuffer())
                m_p.append(p)
            ...
            for i in range(v_count):
                # Jeśli skończyły się unikalne okładki, odświeżamy pulę
                if not available_covers:
                    available_covers = list(c_p)
                    random.shuffle(available_covers)
                
                # Wyciągamy jedną okładkę (pop() usuwa ją z listy)
                ...
                clips = [ImageClip(process_image_916(p)).set_duration(speed) for p in [chosen_cover] + batch_p]
